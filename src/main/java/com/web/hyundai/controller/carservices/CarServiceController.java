...
public class CarServiceController {
    ...
    private final Set<String> EXCLUDED_FIELDS = Stream.of("titleGEO", "textGEO", "nameGEO").collect(Collectors.toSet());
    ...
    @GetMapping(path = "/api/services/title/get/{serviceid}", produces = {"application/json;**charset=UTF-8**"})
    public ResponseEntity<String> getServiceTitles(
        ...
        if (service.isPresent()) {
            ...
            if (lang.equals("ka")) title = service.get().getTitleGEO();
    ...
    public ResponseEntity<String> getServices(
        ...
        return ResponseEntity.ok(carBuildService.jsonExcludeFilter(JSON_FILTER_NAME, EXCLUDED_FIELDS, services));
    }
    ...
    public ResponseEntity<String> getServiceTitle(
        ...
        services.forEach(carService -> {
            ...
            if (lang.equals("ka")) title = carService.getTitleGEO();
            serviceTitle.add(new CarServiceWeb(carService.getId(), title));
        });


        return ResponseEntity.ok(carBuildService.jsonExcludeFilter(JSON_FILTER_NAME, EXCLUDED_FIELDS, serviceTitle));
    }
    ...
    public ResponseEntity<?> carServiceCreate(
        ...
        if (logo.getSize() > 1) {
            ...
            return ResponseEntity.ok(carBuildService.jsonFilter(JSON_FILTER_NAME,carServiceRepo.save(carService)));
        }
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("სურათის ველი ცარიელია ან მეტადატა არ მოყვება");
    }
    ...
    public ResponseEntity<?> carServiceUpdate(
        ...
        if (carService.isPresent()) {
            ...
            return ResponseEntity.ok(carBuildService.jsonFilter(JSON_FILTER_NAME,carServiceRepo.save(carService.get())));
        }
