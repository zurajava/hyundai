...
public class CarFormController {
    ...
    public String invoiceRegister(@RequestParam Long tireid,
        ...
        emailService.sendFileMessage(carInvoice.getEmail(),"ინვოისი","გამარჯობა, გიგზავნით თქვენსმიერ შერჩეულ კომპლექტაციას",pdfFile);
    ...
    public ResponseEntity<?> testDriveRegister(@PathVariable Long carid,
        ...
        Optional<Car> car = carRepo.findById(carid);
        Optional<CarComplect> carComplect = carComplectRepo.findByEngineId(Long.valueOf(carTestDrive.getEngineId()));
        if (car.isPresent() && carComplect.isPresent()) {
            CarTestDrive testDrive = carTestDriveRepo.save(carTestDrive);
            String str = String.format(TEXT,carTestDrive.getFullName(),car.get().getModel(),carComplect.get().getEngine().getTitle(),carTestDrive.getEmail(),carTestDrive.getNumber());
        ...
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body("ასეთი კომპლექტაცია/მოდელი ვერ მოიძებნა");
    }
