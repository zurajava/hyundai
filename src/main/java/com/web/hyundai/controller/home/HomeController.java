...
public class HomeController {
    ...
    public List<HomeCarWeb> getCarsByModel(@RequestHeader(value = "accept-language", defaultValue = "en")
                                            @ApiParam(value = "ka მოვიდეს თუ ქართულია დეფაულტად არის en")
                                               String lang,
    ...
    public ResponseEntity<String> getSliders(
        ...
        return ResponseEntity.ok(carBuildService.jsonExcludeFilter(JSON_FILTER_NAME,EXCLUDED_FIELDS,sliders));
    ...
    public ResponseEntity<String> getSlider(
        ...
        if (slider.isPresent()) {
            ...
            if (lang.equals("ka")) slider.get().setTitle(slider.get().getTitleGEO());


            return ResponseEntity.ok(carBuildService.jsonExcludeFilter(JSON_FILTER_NAME,EXCLUDED_FIELDS,slider));
        }
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body("სლაიდერი ვერ მოიძებნა");
    }


    @PostMapping(path = "/admin/home/update-slider/{id}",produces = {"application/json;**charset=UTF-8**"})
    public ResponseEntity<String> updateSlider(@PathVariable
        ...
        if (slider.isPresent()) {
            ...
            if (file != null && file.getSize() > 0) {
                ...
                if (delFile.exists() && !delFile.getName().equals("sample.png")) delFile.delete();
                FileUpload fileUpload = imageService.uploadImage(file, Path.home);
                slider.get().setImage(imageService.removeDoubleSlash(Path.home + fileUpload.getFile().getName()));
            }
